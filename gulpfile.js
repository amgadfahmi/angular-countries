...
function getTemplateCache() {
  return gulp.src(templates)
    .pipe(plumber())
    .pipe(templateCache({
      module: 'angular-data-service.directives'
    }));
}
...
gulp.task('build', function() {
  return eventStream.merge(gulp.src(sourceFiles), getTemplateCache())
    .pipe(plumber())
    .pipe(concat('angular-data-service.js'))
    .pipe(gulp.dest('./dist/'))
    .pipe(uglify())
    .pipe(rename('angular-data-service.min.js'))
    .pipe(gulp.dest('./dist'));
});
...
gulp.task('build-stylesheets', function() {
  return gulp.src(stylesheets)
    ...
    // .pipe(sass())
    .pipe(concatCss('angular-data-service.css'))
    .pipe(gulp.dest('./dist'));
});
